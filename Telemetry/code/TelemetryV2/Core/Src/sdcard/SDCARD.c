...
void openAllFiles() {
	...
	if (statusRegister.SDCARD == SENSOR_OK) {
		...
		if (statusRegister.DamperLF == SENSOR_OK) {
			openFile(damperLFSensor.File, damperLFSensor.path,
					FILE_DEFAULT_MODE);
		}
...
int createHeaders(FIL *file, char *path) {
	...
	if (strstr(path, "IMU") != NULL) {
		...
		for (int i = 0; i < 784; i++) {
			...
			fres = fres
					| f_write(file, headerText, strlen(headerText),
							&bytesWritten);
		...
		fres =
				f_write(file,
						"LOG,utc,pos status,lat,lat dir,lon,lon dir,speed,,track,date,,mag var,var dir,mode ind,chs,ter\r\n",
						strlen(
								"LOG,utc,pos status,lat,lat dir,lon,lon dir,speed,,track,date,,mag var,var dir,mode ind,chs,ter\r\n"),
						&bytesWritten);
	} else {
...
void adcSaveData(ADCSensor *sens) {
	...
	char timestamp[25];
	getTimeInUTCFormat(timestamp);
	int dataLength = sprintf(dataBuffer, "%d,%d,%hu\r\n", sens->timestamp,
			sens->ID, sens->data);
	f_write(sens->File, dataBuffer, dataLength, &writedBytes);
}
void sdFlush() {
